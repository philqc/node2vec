...
import src.learn_features as learn_features
from src.config import logging, ArxivData
...
def create_features(samples, dic_emb):
    f = np.zeros((len(samples), 128))
    lab = np.zeros(len(samples))
    idx = 0
    for s in samples:
        f1 = dic_emb[s[0]]
        f2 = dic_emb[s[1]]
...
def main():
    # create graph
    g = create_graph(ArxivData.EDGE_CSV)
    ...
    df.to_csv(ArxivData.REDUCED_EDGE_CSV, header=False, index=False)
    ...
    with open(ArxivData.FEATURES_FILE, 'rb') as f:
